...
menu_cd = CallbackData("show_menu", "level", "state", "index",
                       "rez", "pre_level", "rez_page_id","test_pre_level","test_level")
...
def make_callback_data(level, state="0", rez="0", index="0",
                       pre_level="0", rez_page_id="0", test_pre_level="0", test_level="0"):
    return menu_cd.new(level=level, state=state, rez=rez, index=index,
                       pre_level=pre_level, rez_page_id=rez_page_id, test_pre_level=test_pre_level, test_level=test_level)
...
async def categories_keyboard(current_level, sql, user_id, test_pre_level,test_level):
    ...
    for category in categories:
        ...
        callback_data = make_callback_data(
            ...
            test_pre_level=category[3],
            test_level=category[4])
